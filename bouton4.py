...
def beautyMouth(mouth,largeurbouche):  # largeur de la bouche puis largeur du visage mesurée a la heuteur de la bouche
    ...
    if (abs(mouth-f)<marge):
        ...
        print ("Votre score de perfection actuel est de ",perfect, "%","\n","\n")
    if perfect == 0:
        print("vous êtes mathematiquement moche", "\n")
...
cadre = tk.Label(root, relief=RAISED, cursor="tcross")
cadre.bind("<Button-1>", pointeur)
cadre.bind("<Motion>", pointeurz)
cadre.pack()


But=Tkinter.Button(root,text='Open',relief=RAISED, cursor="star",command=lambda :Open_file(cadre))
